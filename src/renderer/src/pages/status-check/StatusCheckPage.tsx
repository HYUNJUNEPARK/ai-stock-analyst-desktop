...
export default function StatusCheckPage(): React.JSX.Element {
  const navigate = useNavigate()
  const { selectedModel } = useApp()

  useEffect(() => {
    if (import.meta.env.DEV) console.log('[Page] StatusCheckPage 렌더링')
  }, [])
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
        ...
        <p style={{ color: 'var(--text-secondary)', fontSize: 'var(--text-sm)' }}>분석 환경을 확인하고 있습니다...</p>
      </div>
    </div>
  )
}
