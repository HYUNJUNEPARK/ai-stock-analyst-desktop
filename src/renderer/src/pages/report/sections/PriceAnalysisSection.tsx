    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 6 }}>
            {([['1W', data.currentPrice.returns['1w']], ['1M', data.currentPrice.returns['1m']], ['3M', data.currentPrice.returns['3m']], ['YTD', data.currentPrice.returns.ytd]] as [string, string][]).map(([label, val]) => (
              <div key={label} style={{ border: '1px solid var(--border)', borderRadius: 8, padding: '6px 8px', textAlign: 'center', background: '#fff' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        <div style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', background: '#fff', borderBottom: '1px solid var(--border)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)' }}>MACD</div>
          <div style={{ padding: '10px 14px', background: '#fff', display: 'flex', gap: 12 }}>
          ...
          <div style={{ padding: '8px 14px', fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', lineHeight: 1.6, borderTop: '1px solid var(--border)', background: '#fff' }}>
      ...
      {/* 볼린저 밴드 */}
      <div style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', background: '#fff', borderBottom: '1px solid var(--border)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)' }}>볼린저 밴드 (20일, 2σ)</div>
        <div style={{ padding: '10px 14px', background: '#fff', display: 'flex', gap: 12 }}>
      ...
      {/* 거래량 */}
      <div style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', background: '#fff', borderBottom: '1px solid var(--border)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)' }}>거래량 분석</div>
        <div style={{ padding: '10px 14px', background: '#fff', display: 'flex', gap: 12 }}>
      ...
      {data.chartPattern?.name && data.chartPattern.name !== '특이 패턴 없음' && (
        <div style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ padding: '10px 14px', background: '#fff', borderBottom: '1px solid var(--border)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)' }}>차트 패턴</div>
      ...
      {/* 기술적 종합 판정 */}
      <div style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
        <div style={{ padding: '10px 14px', background: '#fff', borderBottom: '1px solid var(--border)', fontSize: 'var(--text-sm)', fontWeight: 700, color: 'var(--text-primary)' }}>기술적 종합 판정</div>
        <div style={{ padding: '10px 14px', background: '#fff', display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {Object.entries({ '추세': signals.trend, '모멘텀': signals.momentum, '변동성': signals.volatility, '거래량': signals.volume, '지지/저항': signals.supportResistance }).map(([label, val]) => (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '4px 10px', border: '1px solid var(--border)', borderRadius: 8, background: '#fff' }}>
        ...
        {data.technicalSummary.entryPrice && (
          <div style={{ padding: '8px 14px', borderTop: '1px solid var(--border)', background: '#fff' }}>
        ...
        <div style={{ padding: '10px 14px', fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', lineHeight: 1.65, borderTop: '1px solid var(--border)', background: '#fff' }}>
