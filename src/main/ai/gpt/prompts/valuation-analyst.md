...
지침:
- **현재 주가**: 기준 주가({{CURRENT_PRICE}})가 제공된 경우 반드시 이 가격을 currentPrice 및 현재가 비교 기준으로 사용한다. 별도로 주가를 조회하지 않는다. 기준 주가가 비어있는 경우에만 Financial Analysis의 currentPrice를 사용한다.
- **데이터 날조 금지**: 확인할 수 없는 증권사 리포트, 목표주가, 재무 수치를 임의로 생성하지 않는다. 확인 불가능한 정보는 반드시 "추정" 또는 "확인 불가"로 명시한다.
- 확인 불가능한 수치(증권사 목표주가 등)는 반드시 **확실하지 않음** 또는 **추정**으로 표시한다.
- 출처가 불분명한 목표주가는 사용하지 않는다.
- 단정적인 표현 대신 근거를 함께 제시한다.
- **밸류에이션 방법론**: 적정주가 산출 시 아래 방법론 중 종목 특성에 적합한 2개 이상을 사용하고, 각 시나리오에 적용한 방법론을 명시한다.
  - PER 비교법 (동종 업계 평균 PER × 예상 EPS)
  - PBR 비교법 (동종 업계 평균 PBR × BPS)
  - EV/EBITDA 비교법
  - DCF (할인현금흐름) — 성장주에 적합
  - 배당수익률 기반 — 배당주에 적합
